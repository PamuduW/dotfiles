...
test_python_component_owns_default_global_packages() {
	local expected=(
		python3 python3-pip python3-venv python3-dev pipx
		python3-pil python3-lxml python3-openpyxl python3-docx
		python3-yaml python3-requests python3-bs4 python3-jsonschema
	)
	local python_idx
	python_idx="$(comp_index_of python)" || return 1
	[[ "${COMP_LABELS[$python_idx]}" == 'Python packages (runtime + common libraries)' ]] || return 1
	[[ "${COMP_PLAN_DETAILS[python]:-}" == '13 apt packages' ]] || return 1
...
test_package_metadata_has_exact_66_with_descriptions() {
	declare -F package_metadata_load >/dev/null || return 1
	package_metadata_load "$PKG_FILE" || return 1
	[[ "${#PACKAGE_LIB_NAMES[@]}" -eq 66 ]] || return 1
	local expected=(
		git curl ca-certificates bash-completion bubblewrap stow shellcheck shfmt tree build-essential pkg-config cmake
		python3 python3-pip python3-venv python3-dev pipx
		python3-pil python3-lxml python3-openpyxl python3-docx
		python3-yaml python3-requests python3-bs4 python3-jsonschema
		duf ripgrep fd-find fzf zoxide eza jq gh moreutils git-lfs tmux sqlite3
